...
export const createProviderService = async (req, res) => {
  try {
    ...
    if (!hourlyRate && !dailyRate) {
      return res.status(400).json({ error: 'At least one of hourlyRate or dailyRate must be provided' });
    }
    ...
    const existingService = await ProviderService.findOne({ provider: providerId, template });
...
export const updateProviderService = async (req, res) => {
  try {
    ...
    const service = await ProviderService.findOne({ _id: serviceId, provider: providerId });
    if (!service) {
...
export const deleteProviderService = async (req, res) => {
  try {
    ...
    const service = await ProviderService.findOneAndDelete({ _id: serviceId, provider: providerId });
...
export const getAllTemplateNames = async (req, res) => {
  try {
    ...
    return handleResponse(res,200,"All templates fetched successfully",templates)
  } catch (error) {
